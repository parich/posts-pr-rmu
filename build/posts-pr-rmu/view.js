...
function bringSearchToLife(el) {
  ...
  function refreshResults() {
    ...
    if (activeTab) {
      fetchAndRenderPosts(activeTab.dataset.slug, input ? input.value.trim() : "", resultsContainer, currentPage, paginationContainer);
    }
  }
...
async function fetchAndRenderPosts(slug, searchTerm, container, page = 1, paginationContainer) {
  try {
    ...
    const query = `${BASE_URL}wp-json/wp/v2/posts?categories=${catId}&search=${encodeURIComponent(searchTerm)}&page=${page}&per_page=8&_embed`;
...
async function getCategoryIdBySlug(slug) {
  const res = await fetch(`${BASE_URL}wp-json/wp/v2/categories?slug=${encodeURIComponent(slug)}`);
