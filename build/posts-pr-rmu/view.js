...
const CATEGORY_SLUGS = [{
  ...
  slug: "จดหมายข่าวพระวรุณ",
  name: "จดหมายข่าวพระวรุณ"
}, {
  slug: "ข่าวประชาสัมพันธ์",
...
function bringSearchToLife(el) {
  ...
  function refreshResults() {
    ...
    if (activeTab) {
      fetchAndRenderPosts(activeTab.dataset.slug, input ? input.value.trim() : "",
      // แก้ตรงนี้
      resultsContainer, currentPage, paginationContainer);
    }
  }
...
async function fetchAndRenderPosts(slug, searchTerm, container, page = 1, paginationContainer) {
  try {
    ...
    const query = `https://pr.rmu.ac.th/wp-json/wp/v2/posts?categories=${catId}&search=${encodeURIComponent(searchTerm)}&page=${page}&per_page=8&_embed`;
    ...
    if (posts.length) {
      ...
      renderPagination(paginationContainer, totalPages, page, newPage => {
        fetchAndRenderPosts(slug, searchTerm, container, newPage, paginationContainer);
      });
    } else {
...
async function getCategoryIdBySlug(slug) {
  const res = await fetch(`https://pr.rmu.ac.th/wp-json/wp/v2/categories?slug=${encodeURIComponent(slug)}`);
  const categories = await res.json();
  return categories[0]?.id || null;
}
function generateHTML(posts) {
  return dompurify__WEBPACK_IMPORTED_MODULE_0__["default"].sanitize(posts.map(post => {
    const image = post._embedded?.["wp:featuredmedia"]?.[0]?.source_url || "";
    return `
          <a href="${post.link}" class="card" target="_blank">
            ${image ? `<img src="${image}" alt="${post.title.rendered}" class="card-image"/>` : ""}
            <div class="card-body">
...
function renderPagination(container, totalPages, currentPage, onPageClick) {
  ...
  // ปุ่มก่อนหน้า
  if (currentPage > 1) {
    buttons += `<button class="pagination-btn" data-page="${currentPage - 1}">&lt;</button>`;
  }
